...
def execute(args, master_command, test_id=-1):
    if len(args.works) != 0:
        pool = ThreadPool(len(args.works))
        worker_command = "nice -n -20 /root/distributed-llama/main worker --port 9998 --nthreads {}".format(args.thread)

        for worker_ip in args.works:
            save_log_without_stdout = partial(save_log, save_path = os.path.join(args.save_folder,"worker_{}.log".format(worker_ip)), test_id = test_id)
            pool.apply_async(ssh_worker_execmd, args=(worker_ip, 22, "root", "123", worker_command), callback=save_log_without_stdout)    
        pool.close() 
    ...
    save_log(stdout=out, save_path=os.path.join(args.save_folder,"master.log"), test_id=test_id)
...
def test(args):
    ...
    for _ in range(args.warm_up):
        ...
        print(execute(args, master_command))
    ...
    for i in range(args.loop):
        ...
        out = execute(args, master_command, i)
        res = out.split("\n")[-4:-1]
        print(res)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--works', type=list, default=["192.168.6.1","192.168.6.2","192.168.6.4"], help="the ip of workers")
    ...
    parser.add_argument('--model', type=str, default="dllama_llama-2-13b_q40",help="the model")
